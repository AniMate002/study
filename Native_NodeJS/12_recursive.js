...
async function fibonacciNonBlocking(number) {
    return new Promise((resolve, reject) => {
        ...
        setImmediate(() => {
            Promise.all([
                fibonacciNonBlocking(number - 1),
                fibonacciNonBlocking(number - 2),
            ]).then(([first, second]) => resolve(first + second));
        });
    });
}

fibonacciNonBlocking(6).then((result) =>
    console.log("Fibonacci Non-Blocking  number at position 6: ", result)
);
...
async function cachFibonacciNonBlocking(number) {
    return new Promise((resolve, reject) => {
        ...
        setImmediate(() => {
            Promise.all([
                cachFibonacciNonBlocking(number - 1),
                cachFibonacciNonBlocking(number - 2),
            ]).then(([first, second]) => {
...
cachFibonacciNonBlocking(6).then((result) =>
    console.log("Fibonacci Cached Non-Blocking number at position 6: ", result)
);
